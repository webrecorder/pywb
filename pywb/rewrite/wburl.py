...
import six
from six.moves.urllib.parse import quote, quote_plus, unquote_plus, urlsplit, urlunsplit
...
class BaseWbUrl(object):
    ...
    def __init__(self, url="", mod="", timestamp="", end_timestamp="", type_=None):
        self.url = url
...
class WbUrl(BaseWbUrl):
    ...
    @staticmethod
    def to_wburl_str(
        url, type_=BaseWbUrl.LATEST_REPLAY, mod="", timestamp="", end_timestamp=""
    ):

        if WbUrl.is_query_type(type_):
            tsmod = mod + "/" if mod else ""
            tsmod += timestamp + "*" + end_timestamp + "/" + url
            if type_ == BaseWbUrl.URL_QUERY:
                return tsmod + "*"
            return tsmod
    ...
    @property
    def is_embed(self):
        return self.mod and self.mod not in self.EMBEDED_MODS
